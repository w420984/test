...
public class TwistView extends View {
	...
	private static final float AUTO_AREA_BOTTOM_RIGHT = 1 / 2F, AUTO_AREA_BOTTOM_LEFT = 1 / 2F;// 右下角和左侧自滑区域占比
	private static final float AUTO_AREA_BOTTOM = 1 / 5F;
	...
	private float mPointDownX, mPointDownY;	//down时的坐标
	private float mPointX, mPointY;// 手指触摸点的坐标
	private float mValueAdded;// 精度附减值
	private float mBuffArea;// 底部缓冲区域
	private float mAutoAreaBottom, mAutoAreaRight, mAutoAreaLeft;// 右下角和左侧自滑区域
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		mAutoAreaBottom = location[1] + mViewHeight * (1 - AUTO_AREA_BOTTOM);
	...
	private void drawBitmaps(Canvas canvas) {
		...
		mPageIndex = mPageIndex >= mBitmaps.size() ? 0 : mPageIndex;
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		switch (ev.getAction()) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				xLast = curX;
				yLast = curY;
				if (curY > mAutoAreaBottom && xDistance > yDistance){
					//当在翻页区域横向滑动时,由view来处理滑动事件，父控件不要插手
					getParent().requestDisallowInterceptTouchEvent(true);
				}
				break;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_OUTSIDE:
			if (mPointDownX < mAutoAreaLeft || mPointDownY < mAutoAreaBottom){
